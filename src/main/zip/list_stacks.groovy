// --------------------------------------------------------------------------------
// List all of the Solution Stack Names for an Elastic Beanstalk Environment and allow the user to
// select one.
// --------------------------------------------------------------------------------
...
println "Stack Filter: ${stackFilter}"
...
try {
    ...
    if (solutionStacks == null || solutionStacks.size() == 0) {
        println "Error: No Solution Stacks found";
        System.exit(1);
    }

    List<Properties> stackList = new ArrayList<Properties>();
    ...
    for (int i = 0; i < solutionStacks.size(); i++) {
        ...
        if (stackName.toLowerCase().contains(stackFilter.toLowerCase())) {
            ...
            if (debugMode) {
                helper.log("Skipping solution stack: ${stackName} as it does not match filter")
            }
        }
    }

    helper.log("Found ${stackList.size()} matching solution stacks out of ${solutionStacks.size()} in total")

    apTool.setOutputProperty("OUTPUT_DATA", stackList);
